...
<div class="card card-body bg-light mt-5">
    ...
    <form action="<?php echo URLROOT; ?>/posts/edit/<?php echo $data['id']; ?>" method="post">
        <div class="form-group">
            ...
            <span class="invalid-feedback<?php echo (!empty($data['title_err'])) ? ' d-block' : ''; ?>"><?php echo $data['title_err']; ?></span>
        </div>
        <div class="form-group">
            <label for="body">Body: <sup>*</sup></label>
            <textarea name="body" class="form-control <?php echo (!empty($data['body_err'] ? 'is-invalid' : '')); ?>"><?php echo $data['body']; ?></textarea>
            <span class="invalid-feedback<?php echo (!empty($data['body_err'])) ? ' d-block' : ''; ?>"><?php echo $data['body_err']; ?></span>
        </div>
